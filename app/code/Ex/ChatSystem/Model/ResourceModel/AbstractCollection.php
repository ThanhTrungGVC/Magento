...
namespace Ex\ChatSystem\Model\ResourceModel;
...
abstract class AbstractCollection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    protected function performAfterLoad($tableName, $columnName)
    {
        ...
        if (count($items)) {
            ...
            $select = $connection->select()->from(['exchatsystem_entity_store' => $this->getTable($tableName)])
                ->where('exchatsystem_entity_store.' . $columnName . ' IN (?)', $items);
     ...
     protected function performAfterLoadUser($tableName, $columnName)
    {
        ...
        if (count($items)) {
            $connection = $this->getConnection();
            $select = $connection->select()->from(['exchatsystem_entity_user' => $this->getTable($tableName)])
                ->where('exchatsystem_entity_user.' . $columnName . ' IN (?)', $items);
    ...
    protected function performAfterLoadDepartment($tableName, $columnName)
    {
        ...
        if (count($items)) {
            $connection = $this->getConnection();
            $select = $connection->select()->from(['exchatsystem_permission_department' => $this->getTable($tableName)])
                ->where('exchatsystem_permission_department.' . $columnName . ' IN (?)', $items);
            $result = $connection->fetchAll($select);
            $data = [];
            foreach ($result as $key => $_result) {
    ...
    protected function performAfterLoadCategory($tableName, $columnName)
    {
        ...
        if (count($items)) {
            $connection = $this->getConnection();
            $select = $connection->select('category_id')->from(['exchatsystem_department_category' => $this->getTable($tableName)])
                ->where('exchatsystem_department_category.' . $columnName . ' IN (?)', $items);
            $result = $connection->fetchAll($select);
            $data = [];
            foreach ($result as $key => $_result) {
